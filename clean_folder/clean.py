...
files_by_type = {'images': ('JPEG', 'PNG', 'JPG', 'SVG', 'BMP'),
...
def move_file(file, file_path):
    normalize_file_name = normalize(file)
    file_name, file_extention = os.path.splitext(normalize_file_name)
    file_format_sorting(normalize_file_name, file)
    for key, value in files_by_type.items():
        new_path = f'{PATH}\\{key}'
        if normalize_file_name.upper().endswith(value):
            ...
            if key == 'archives':
                ...
                shutil_move(file_path, f'{new_path}\\{normalize_file_name}')
        elif not normalize_file_name.upper().endswith(value):
